# Rase
## A Radio Sensor Project

### Disclaimer of Liability

This project is licensed under the terms of the GNU General Public License, version 3 or later (the "License"). A copy of the license is included in the root directory of this repository, and can also be found at "https://www.gnu.org/licenses/gpl-3.0.en.html".

This project is provided "as is", as open-source software and hardware, and is intended for hobbyist and enthusiast use, without warranties of any kind, either express or implied. The authors and contributors disclaim all liability for any damages or losses arising from the use or inability to use this project, including but not limited to direct, indirect, incidental, special, or consequential damages.

You are solely responsible for:
- Ensuring compliance with local laws and regulations when using this project.
- Any modifications or adaptations made to the hardware, firmware, or software.
- The safe and proper operation of any devices built using this project.
- Compliance with applicable licensing terms (e.g., GPL, MIT).

No warranties are given regarding:
- The accuracy, reliability, or suitability of the project for any purpose.
- The absence of errors or defects in the code, documentation, or hardware designs.
- Compatibility with third-party software or hardware.
- Build guides and documentation are provided for informational purposes. The authors do not guarantee correctness, completeness, or suitability for any specific purpose.

By using this project, you acknowledge and agree to these terms.

